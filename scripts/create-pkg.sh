...
# create-pkg.sh - Create a signed macOS pkg installer for marvel
...
cp "$BINARY_PATH" "$STAGING_DIR/usr/local/bin/marvel"
chmod +x "$STAGING_DIR/usr/local/bin/marvel"

pkgbuild --root "$STAGING_DIR" \
  --identifier com.arcavenae.marvel \
  --version "$VERSION" \
  --install-location / \
